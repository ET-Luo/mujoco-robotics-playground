...
Only the basic simulation is implemented; subsequent stages are learning placeholders.
...
Dependencies are limited to `mujoco` for physics, `numpy` for numerical learning,
and `matplotlib` for future plots. The latter two are reserved for later exercises.
